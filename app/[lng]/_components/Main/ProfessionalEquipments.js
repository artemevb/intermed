...
//               <a href={`/${lng}/categories`} className="border px-12 py-3 hover:bg-[#E94B50] hover:text-[#fff] font-bold">
//                 {t('more-products')}
//               </a>
//             </div>
//           </div>
...
const EquipmentCarousel = () => {
  ...
  const staticProducts = [
    {
      ...
      sale: false,
      popular: true,
      gallery: [{ url: "/images/product1.jpg" }],
    ...
    },
    {
      ...
      sale: true,
      popular: false,
      gallery: [{ url: "/images/product2.jpg" }],
  ...
  useEffect(() => {
    // Вместо API используем статические данные
    setFilteredData(staticProducts);
    setIsMounted(true);
  }, []);
  ...
  const handleFilter = useCallback((category) => {
    ...
    if (category === "all") {
      ...
      setFilteredData(staticProducts.filter((item) => item.new));
    } else if (category === "promotions") {
      setFilteredData(staticProducts.filter((item) => item.popular));
    }
  }, [staticProducts]);
