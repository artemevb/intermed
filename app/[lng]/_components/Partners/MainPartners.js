...
export default function PartnerPage() {
    ...
    useEffect(() => {
      const fetchPartnerInfo = async () => {
        try {
          const response = await axios.get(`http://213.230.91.55:8130/v1/partner/${slug}`, {
        ...
        <div className="w-full max-w-[1440px] mx-auto px-4 py-10">
                <div key={partner.id} className='mb-[60px] mdl:mb-[75px] xl:mb-[95px]'>
                    <div className='border-b pb-[25px]'>
                        <h1 className="text-[25px] mdx:text-[30px] mdl:text-[35px] xl:text-[40px] font-semibold mb-2">{partner.name}</h1>

                        <h2 className="text-[12px] max-w-[820px] mdx:text-[16px] text-[#808080] font-semibold mb-4">{partner.note}</h2>
                        <p className=" whitespace-pre-line mt-[20px] xl:mt-[40px] text-[15px] mdx:text-[18px] mdl:text-[18px]">{partner.about}</p>
                    </div>
