...
import { languages, fallbackLng } from '../i18n/settings'; // Импортируем fallbackLng
...
export function generateMetadata({ params: { lng } }) {
    ...
    const imageUrl = `${baseUrl}/og.jpg`; // Абсолютный URL изображения

    // Формируем абсолютный URL страницы с учетом языка
    const currentUrl = `${baseUrl}${
        lng && lng !== fallbackLng ? `/${lng}` : ''
    }`;
